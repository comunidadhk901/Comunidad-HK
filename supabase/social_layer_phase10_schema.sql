...
--
-- Foto para eventos (se sube al crearlos desde el feed) + una seccion
-- real de "Eventos" que lista todos los eventos creados.

alter table events add column if not exists photo_updated_at timestamptz;

insert into storage.buckets (id, name, public)
values ('event-photos', 'event-photos', true)
on conflict (id) do nothing;

drop policy if exists "cualquiera ve event-photos" on storage.objects;
create policy "cualquiera ve event-photos" on storage.objects for select to anon, authenticated using (bucket_id = 'event-photos');

drop policy if exists "autenticados suben event-photos" on storage.objects;
create policy "autenticados suben event-photos" on storage.objects for insert to authenticated with check (bucket_id = 'event-photos');

-- Solo quien creo el evento puede actualizarlo (necesario para poder
-- guardar photo_updated_at despues de subir la foto).
drop policy if exists "creador actualiza evento" on events;
create policy "creador actualiza evento" on events for update to authenticated
  using (created_by_id = my_member_id()) with check (created_by_id = my_member_id());
...
begin
  ...
  from (
    select ev.id, ev.title, ev.description, ev.event_date, ev.location, ev.created_at, ev.created_by_id,
      ...
      (select count(*) from event_rsvps r where r.event_id = ev.id) as rsvp_count,
      exists(select 1 from event_rsvps r2 where r2.event_id = ev.id and r2.member_id = my_id) as rsvped_by_me
    from events ev
...
-- get_feed(): agrega la foto del evento a lo que ya devuelve para un post
-- de tipo evento (para que tambien se vea en el feed, no solo en Eventos).
